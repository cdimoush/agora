...
def compose_service_block(agent_dir: Path) -> dict:
    ...
    if wt_dir.is_dir():
        ...
        if gh_config.is_dir():
            volumes.append(f"{gh_config}:/home/agent/.config/gh:ro")
...
def init_agent(name: str) -> Path:
    """Create a new agent via git worktree.

    Creates ../agora-<name> as a worktree on branch worktree/<name>.
    Returns the worktree path.
    """
    slug = _validate_name(name)
    return worktree_create(slug)
...
def _worktree_path(name: str) -> Path:
    """Return the worktree path for a given agent name.

    Worktrees live alongside the repo: ../agora-<name> relative to the repo root.
    """
    repo_root = Path.cwd().resolve()
    return repo_root.parent / f"agora-{name}"


def worktree_create(name: str) -> Path:
    """Create a git worktree for an agent.

    Creates ../agora-<name> on branch worktree/<name> from main.
    Returns the worktree path.
    """
    import subprocess as _sp
...
def worktree_status() -> None:
    ...
    # Parse worktree paths (skip the main repo itself)
    worktrees = []
    repo_root = Path.cwd().resolve()
    for line in result.stdout.splitlines():
        if line.startswith("worktree "):
            wt_path = Path(line.split(" ", 1)[1])
            if wt_path.resolve() != repo_root:
                worktrees.append(wt_path)

    if not worktrees:
        print("No worktrees found. Use 'agora worktree create <name>' to create one.")
        return
...
def main(argv: list[str] | None = None) -> None:
    ...
    # -- init --
    init_parser = sub.add_parser("init", help="Create a new agent via worktree")
    ...
    if args.command == "init":
        if not args.name:
            print("Error: name is required. Usage: agora init <name>", file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            print(f"  3. Edit {path}/agent/agent.yaml and {path}/agent/CLAUDE.md")
            print(f"  4. bash {path}/agora.sh")
