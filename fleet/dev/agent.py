"""Dev — autonomous developer agent for AgoraGenesis.

Dual-mode: DM (dev tasks) and channel (social citizen).
Uses Claude CLI with mode-specific budgets and cwd.
"""
...
class Dev(Agora):
    ...
    async def _handle_channel(self, message: Message) -> str | None:
        """Social mode: citizen behavior like Rex/Nova."""
        logger.info(
    ...
    async def _call_claude(self, prompt: str, dev_mode: bool = False) -> str | None:
        ...
        budget = "1.00" if dev_mode else "0.25"
        timeout = 180 if dev_mode else 60
        ...
        with self.span("llm_call", model="sonnet", prompt_length=len(prompt), dev_mode=dev_mode) as s:
...
if __name__ == "__main__":
    ...
    config_path = "config/agent.yaml" if Path("config/agent.yaml").exists() else "agent.yaml"
    bot = Dev.from_config(config_path)

    async def _main():
        await bot.start()
        bot.watch_config(config_path)
        try:
            await bot._run_task
